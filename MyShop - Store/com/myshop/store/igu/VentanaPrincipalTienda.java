...
public class VentanaPrincipalTienda extends JFrame {
		...
		Action action = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (comprobarEntero(tcl.getNewValue().toString())) {
					...
					if (nuevoValor > stock) {
						JOptionPane.showMessageDialog(derecha, "No hay tantos artï¿½culos en stock", "Error",
	...
	public void cargarCategorias(String nombreCat){
		...
		if(cat.isEmpty()){
			...
			texto.setText("Esta categorï¿½a no contiene productos");
			getPanelProductos().removeAll();
						...
						for (int i = 0; i < row; i++) {
							...
							if (p.getID() == cod) {
								if (p.getStock() > Integer.parseInt(table.getValueAt(i, 2).toString())) {
									...
									JOptionPane.showMessageDialog(derecha, "No se pueden aï¿½adir mï¿½s artï¿½culos", "Error",
											JOptionPane.ERROR_MESSAGE);
								}
								seguir = false;
